...
# YOLO — auto downloads yolov8n.pt (small, fast, good enough for person detection)
print("\n2. Loading YOLOv8 (auto download ~6MB)...")
yolo = YOLO('yolov8n.pt')
print("✅ YOLO loaded")

# ═══════════════════════════════════════════════════════════
# CELL 3 — Upload Video 4
# ═══════════════════════════════════════════════════════════
print("\n3. Upload Video 4 (two players side by side)")
uv4 = files.upload()
V4_VIDEO = list(uv4.keys())[0]
print(f"✅ {V4_VIDEO}")

# ═══════════════════════════════════════════════════════════
# CELL 4 — Constants + Helper Functions
# ═══════════════════════════════════════════════════════════
...
# ── Constants (identical to c_a__1_.py) ──────────────────
SELECTED_JOINTS = [11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27]
WINDOW_SIZE     = 30
THRESHOLD       = 0.82
STRIDE          = 5
COOLDOWN_FRAMES = 180   # ~3s at 60fps
...
STROKE_COLORS = {
    0: (0,   200,  80),   # Forehand — green
    1: (0,    80, 200),   # Backhand — orange
    2: (200,   0,  80),   # Serve    — purple
}
...
def detect_ball_hsv(frame):
    """HSV ball detection on full frame."""
...
def extract_skeleton_from_crop(pose_detector, frame, box):
    """
    Crop frame to bounding box, run MediaPipe Tasks pose,
    return 11 joints in FULL-FRAME normalised coordinates.
    """
    x1, y1, x2, y2 = box
...
def hip_normalise_and_predict(buffer):
    """
    Exact same preprocessing as c_a__1_.py.
    buffer: list of 30 x (22,) flattened joint arrays
...
# ═══════════════════════════════════════════════════════════
# CELL 5 — Single-Pass Extraction + Detection
# YOLO detects: persons (0) + sports ball (32) + racket (38)
# ═══════════════════════════════════════════════════════════

# ── Constants ──────────────────────────────────────────────
SELECTED_JOINTS = [11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27]
WINDOW_SIZE     = 30
THRESHOLD       = 0.70   # lowered from 0.82 for court video
STRIDE          = 5
COOLDOWN_FRAMES = 180
...
while cap.isOpened():
    ...
    # ── 1. YOLO — detect persons + ball + racket ────────────
    ...
    # ── 2. Skeleton extraction per player ──────────────────
    player_skels = {}
    ...
    # ── 3. Ball near player? (YOLO-based) ───────────────────
    def ball_near_player_yolo(ball_pos, player_box):
        """Ball within player bounding box expanded by 20%"""
    ...
    # ── 4. Racket near player? ───────────────────────────────
    def racket_near_player(racket_centers, player_box):
        """Any racket center inside player bounding box"""
    ...
    # ── 5. CNN-LSTM sliding window per player ──────────────
    for pid in ['P1', 'P2']:
        ...
        if (len(buffers[pid]) == WINDOW_SIZE
            ...
            if conf >= THRESHOLD:
                # Gate: ball OR racket near this player
    ...
    # Ball — YOLO detected (green circle)
    if ball_pos:
        bx, by = ball_pos
...
# ═══════════════════════════════════════════════════════════
# CELL 6 — Results Summary + Download Final Video
# ═══════════════════════════════════════════════════════════
